using merge_mansion_update.Il2CppBinary.Extensions;
using merge_mansion_update.Il2CppBinary.Utils;
using Serilog;
using System.Runtime.InteropServices;
using static merge_mansion_update.Il2CppBinary.ExecutableFormats.ElfConstants;
        ...
        */
        ...
        protected override void Load()
        {
            ...
            if (!IsDumped)
            {
                ...
                if (CheckProtection())
                {
                    Log.Error("This file may be protected.");
                }
            }
        }
        ...
        public override bool Search()
        {
            ...
            if (resultList.Count == 1)
            {
                ...
                Log.Information("CodeRegistration : {0:x}", codeRegistration);
                Log.Information("MetadataRegistration : {0:x}", metadataRegistration);
        ...
        public override bool SymbolSearch()
        {
            ...
            {
                Log.Information("Detected Symbol !");
                Log.Information("CodeRegistration : {0:x}", codeRegistration);
                Log.Information("MetadataRegistration : {0:x}", metadataRegistration);
                Init(codeRegistration, metadataRegistration);
                return true;
            }
            Log.Error("No symbol is detected");
            return false;
        }
        ...
        {
            Log.Information("Applying relocations...");
            try
            {
        ...
        private bool CheckProtection()
        {
            try
            {
                ...
                foreach (var symbol in symbolTable)
                {
                    ...
                    {
                        case "JNI_OnLoad":
                            Log.Warning("find JNI_OnLoad");
                            return true;
                    }
                }
                if (sectionTable != null && sectionTable.Any(x => x.sh_type == SHT_LOUSER))
                {
                    Log.Warning("find SHT_LOUSER section");
                    return true;
                }
            }
            catch
